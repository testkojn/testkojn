...
{
    QString ret = QString("testkojn:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TSK unit input in lnReqAmount, we generate the URI with TSK as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TestkojnUnits::format(TestkojnUnits::TSK, ui->lnReqAmount->valueAsMpq()));
            paramCount++;
        }
        else
        {
